...
def catOnlyBest():
    ...
    open('recombination/filtering/data/catRecombOnlyBestScoresBeforeCombining.txt','w').write(myOutString)
    ...
    russNull = {}
    russOrigParsToTotal = {}
    with open('recombination/filtering/russ_null.txt') as f:
        for line in f:
            splitLine = (line.strip()).split()
    ...
    robNull = {}
    robOrigParsToTotal = {}
    with open('recombination/filtering/rob_null.txt') as f:
        for line in f:
            splitLine = (line.strip()).split()
            print(splitLine)
            if len(splitLine) == 1 and splitLine[0].isdigit():
    ...
    with open('recombination/filtering/data/combinedCatOnlyBest.txt') as f:
        for line in f:
            splitLine = (line.strip()).split('\t')
    ...
    open('recombination/filtering/data/combinedCatOnlyBestWithPVals.txt','w').write(myOutString)
