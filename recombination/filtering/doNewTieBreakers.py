...
def applyPval():
    myOutString = ''
    with open('recombination/filtering/data/combinedCatOnlyBestWithPValsFinalReportWithInfSitesNoClusters3SeqP02.txt') as f:
        for line in f:
    ...
    open('recombination/filtering/data/combinedCatOnlyBestWithPValsFinalReportWithInfSitesNoClusters3seqP02RussPval005.txt','w').write(myOutString)
...
def doNewTiebreakers():
    ...
    recombToStringSize = {}
    recombToLines = {}
    with open('recombination/filtering/data/combinedCatOnlyBestWithPValsFinalReportWithInfSitesNoClusters3seqP02RussPval005.txt') as f:
        for line in f:
            splitLine = (line.strip()).split('\t')
                            ...
                            if newLeaves.count(minLeaves) == 1:
                                ...
                                for i in range(0,len(newLeaves)):
                                    if newLeaves[i] == minLeaves:
                                        ...
                                        tempP.append(set([int(newL[i][3]),int(newL[i][6])]))
                                myOutString += joiner(getBiggestBreakpointInterval(tempL))
    open('recombination/filtering/data/combinedCatOnlyBestWithPValsFinalReportWithInfSitesNoClustersNewTiebreak3seqP02RussPval005.txt','w').write(myOutString)
