...
def addPVals():
    keyToP = {}
    with open('recombination/filtering/data/mnk.log') as f:
        for line in f:
    ...
    myOutString = ''
    alreadyUsed = {}
    with open('recombination/filtering/data/allRelevantNodesMNK.txt') as f:
        for line in f:
            splitLine = (line.strip()).split()
            myKey = '_'.join(splitLine[-3:])
    ...
    open('recombination/filtering/data/allRelevantNodesMNKPval.txt','w').write(myOutString)
...
def combinePValueFiles():
    ...
    recombToAB = {}
    recombToPrinted = {}
    with open('recombination/filtering/data/allRelevantNodesMNKPval.txt') as f:
        for line in f:
            splitLine = (line.strip()).split('\t')
            if not splitLine[0].startswith('#'):
    ...
    open('recombination/filtering/data/combinedCatOnlyBestWithAll3PValsTiesBroken.txt','w').write(myOutString)
    open('recombination/filtering/data/combinedCatOnlyBestWithAll3PValsRealTiesBroken.txt','w').write(myOutString2)
...
def addInfSites():
    ...
    trioToInfSites = {}
    with open('recombination/filtering/data/allRelevantNodesInfSites.txt') as f:
        for line in f:
            splitLine = (line.strip()).split('\t')
            trioToInfSites[str(splitLine[0])+'_'+str(splitLine[1])+'_'+str(splitLine[2])] = splitLine[7]

    trioToInfSeq = {}
    with open('recombination/filtering/data/allRelevantNodesInfSeq.txt') as f:
        for line in f:
            splitLine = (line.strip()).split('\t')
    ...
    open('recombination/filtering/data/combinedCatOnlyBestWithPValsFinalReportWithInfSites.txt','w').write(myOutString)
