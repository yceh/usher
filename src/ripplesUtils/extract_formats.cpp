...
void get_parents(Mutation_Annotated_Tree::Tree* T, std::unordered_set<std::string> &need_parents, std::unordered_set<std::string> &all_nodes) {
         ...
         FILE* node_to_parent_fp = fopen("recombination/filtering/data/nodeToParent.txt", "w");
         ...
         for (const auto &node_id: need_parents) {
           ...
           //fprintf(node_to_parent_fp, "%s\n", parent_id.c_str());
					 ...
					 // Temporary formatting for getABABA.py in filtration pipeline (remove "node_")
           fprintf(node_to_parent_fp, "%s\t", node_id.substr(5,node_id.size()).c_str());
