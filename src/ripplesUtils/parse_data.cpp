...
void get_trios(MAT::Tree T, std::string filepath) {
 ...
 for(; !rec.done(); rec.next_line()) {						
   ...
   // Call get_parents to retrieve the parents for all the donors/acceptors placed as siblings
   get_parents(&T, needParent, all_nodes); 

   // Create allRelevantNodeNames.txt file
   FILE* allRelevantNodeNames_fp = fopen("recombination/filtering/data/allRelevantNodeNames.txt", "w");
