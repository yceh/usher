...
int main(int argc, char** argv) {
		...
		const std::string &combined_pvals_filename = "recombination/filtering/data/combinedCatOnlyBestWithPVals.txt";
	  if (argc == 2) { 	
	          input_mat_filename = std::string{argv[1]};
		} else {
						fprintf(stderr, "Error.  Please call like this: build/ripplesUtils <MAT_name.pb>\n");
						exit(1);
		}
    ...
    // Call get_trios -> Ouputs two files:  allRelevantNodeNames.txt and nodeToParent.txt  
