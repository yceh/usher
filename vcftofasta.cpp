...
void parse_lines(char *line_in, char* end, std::vector<pos_nuc> &out) {
    while (line_in != end) {
        ...
        out.push_back(pos_nuc{pos, std::vector<char>()});
...
int main(int argc, char** argv){
    ...
    char * vcf_start=(char*) mmap(0, stat_buf.st_size, PROT_READ, MAP_SHARED, vcf_fd, 0);
    std::perror("");
  ...
  for (int pos_idx=0;pos_idx<pos.size();pos_idx++ ){
    std::fwrite(seq.data()+last_print,1,pos[pos_idx].postions-last_print,stdout);
